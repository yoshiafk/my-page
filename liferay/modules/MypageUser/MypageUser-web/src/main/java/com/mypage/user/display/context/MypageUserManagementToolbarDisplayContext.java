...
public class MypageUserManagementToolbarDisplayContext extends SearchContainerManagementToolbarDisplayContext {
	// Constructor
	public MypageUserManagementToolbarDisplayContext(HttpServletRequest httpServletRequest,
	...
	private DropdownItemList _getOrderBy() {
		if (_getPageType().equals("user")) {
			return new DropdownItemList() {
				{
					add(dropdownItem -> {
						dropdownItem.setActive(Objects.equals(getOrderByCol(), "mypageUserId"));
						dropdownItem.setHref(_getCurrentSortingURL(), "orderByCol", "mypageUserId");
						dropdownItem.setLabel(LanguageUtil.get(request, "Mypage User ID"));
					});
					add(dropdownItem -> {
