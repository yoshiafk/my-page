...
public class UserGroupManagementToolbarDisplayContext extends SearchContainerManagementToolbarDisplayContext {
	// Constructor
	public UserGroupManagementToolbarDisplayContext(HttpServletRequest httpServletRequest,
		...
		_mvcPath = ParamUtil.getString(liferayPortletRequest, "mvcPath", "/user_group/list.jsp");
	}
	...
	private DropdownItemList _getOrderBy() {
		if (_getPageType().equals("category")) {
			return new DropdownItemList() {
				{
					add(dropdownItem -> {
						dropdownItem.setActive(Objects.equals(getOrderByCol(), "mypageUserGroupId"));
						dropdownItem.setHref(_getCurrentSortingURL(), "orderByCol", "mypageUserGroupId");
						dropdownItem.setLabel(LanguageUtil.get(request, "User Group ID"));
					});
					add(dropdownItem -> {
