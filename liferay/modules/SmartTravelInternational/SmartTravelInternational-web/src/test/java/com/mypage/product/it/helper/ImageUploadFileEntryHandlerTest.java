...
public class ImageUploadFileEntryHandlerTest {
    ...
    void setUp() throws Exception {
        ...
        Mockito.when(mockProvider.provide(Mockito.anyString(), Mockito.any())).thenAnswer(invocation -> invocation.getArgument(0));

        // inject ke private field
        Field field = ImageUploadFileEntryHandler.class.getDeclaredField("_uniqueFileNameProvider");
    ...
    void testValidateFile_TooLarge() throws Exception {
        ...
        withStaticMocks(upr, () -> {
            assertThrows(EntryImageSizeException.class, () -> handler.upload(upr));
        }, "image/png");
    }
    ...
    void testValidateFile_InvalidExtension() throws Exception {
        ...
        withStaticMocks(upr, () -> {
            assertThrows(EntryImageNameException.class, () -> handler.upload(upr));
        }, "image/bmp");
    }
    ...
    private void withStaticMocks(UploadPortletRequest upr, Runnable testLogic, String mimeType) throws Exception {
        ...
        try (MockedStatic<ServiceContextFactory> mockedSCF = Mockito.mockStatic(ServiceContextFactory.class);
             MockedStatic<DLFolderLocalServiceUtil> mockedFolder = Mockito.mockStatic(DLFolderLocalServiceUtil.class);
             MockedStatic<DLAppLocalServiceUtil> mockedApp = Mockito.mockStatic(DLAppLocalServiceUtil.class);
             MockedStatic<MimeTypesUtil> mockedMime = Mockito.mockStatic(MimeTypesUtil.class)) {
            ...
            mockedFolder.when(() -> DLFolderLocalServiceUtil.fetchFolder(Mockito.anyLong(), Mockito.anyLong(), Mockito.anyString()))
                        .thenReturn(folder);

            FileEntry fakeEntry = Mockito.mock(FileEntry.class);
            mockedApp.when(() -> DLAppLocalServiceUtil.addFileEntry(
            		Mockito.anyLong(), Mockito.anyLong(), Mockito.anyLong(), Mockito.anyString(), Mockito.anyString(),
            		Mockito.anyString(), Mockito.anyString(), Mockito.anyString(), Mockito.any(File.class), Mockito.any(ServiceContext.class)
