  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.response.status === "SUCCESS") {
        ...
        document.querySelector(".content-message-popup-ispep").textContent = response.response.reason;

        //open modal
        document.getElementById("ispepModal").classList.add("is-open");

        return;
      } else if (response.response.error === "Quota exceeded") {
        //fileInput.value = ''; // Kosongkan input file
        //console.log('response.response.status masuk sini'+response.response.status)

        document.querySelector(".content-message-popup-ispep").textContent = "Unggah foto berhasil";
        ...
        document.querySelector(".content-message-popup-ispep").textContent = response.response.error;
...
function updateNearestFieldsPaspor(fileInput, response) {
  //console.log('updateNearestFields SCAN PASPOR.......>>', response.response);
...
function initDatepickerFromScan(inputClass, date) {
  ...
  //console.log('minDate:', minDate.format('DD-MM-YYYY'), 'maxDate:', maxDate.format('DD-MM-YYYY'));
...
function fetchCountriesDefault() {
  //console.log('fetch country from >>', apiLRCountries);
  return new Promise((resolve, reject) => {
    $.ajax({
...
$(document).ready(function () {
  ...
  if (lId < 1) {
    //console.log('beli pertama, render default country select2');
    getCountries();
  }
  ...
  document.querySelectorAll(".fileInput").forEach((input) => {
    input.addEventListener("change", function (event) {
      ...
      // Trigger ke checkForm jika upload file
      checkForms();

      const containerForm = input.closest(".container-form");
      const negaraElement = containerForm.querySelector(".negara");
      const inputKtpKitas = containerForm.querySelector(".classInputKtpKitas");
