package com.mypage.product.it2.portlet;
...
import com.mypage.product.it2.constants.SmartTravelInternational2PortletKeys;
...
@Component(immediate = true, property = { "com.liferay.portlet.display-category=category.sample",
		...
		"javax.portlet.display-name=Smart Travel International 2", "javax.portlet.init-param.template-path=/",
		"javax.portlet.init-param.view-template=/step1.jsp",
		"javax.portlet.name=" + SmartTravelInternational2PortletKeys.SMARTTRAVELINTERNATIONAL2,
		"javax.portlet.resource-bundle=content.Language",
		"javax.portlet.security-role-ref=power-user,user" }, service = Portlet.class)
public class SmartTravelInternational2Portlet extends MVCPortlet {
...
//		String utm_content = ParamUtil.getString(httpRequest, "utm_content");
		...
		if (!rcParam.isEmpty() && rcParam != null) {
			try {
				...
				renderRequest.setAttribute("rc", rcParam);
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
		}
...
//		renderRequest.setAttribute("utm_content", utm_content);
		...
		String riplayWebContentPrefix = "it6-v2-riplay-personal-";
		
		String _dataHeader = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "informasi-produk",themeDisplay);
		String dataItemFiturUtama = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "fitur-utama", themeDisplay);
		String dataItemManfaat = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "item-manfaat", themeDisplay);
		String dataItemResiko = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "risiko", themeDisplay);
		String dataItemBiaya = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "item-biaya", themeDisplay);
		String dataItemKecuali = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "pengecualian", themeDisplay);
		String dataItemSyarat = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "persyaratan", themeDisplay);
		String dataItemSimulasi = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "simulasi", themeDisplay);
		String dataItemInfoTambahan = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "informasi-tambahan", themeDisplay);
		String dataItemCatatanPenting = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "catatan-penting", themeDisplay);
		String dataItemDisclaimer = helpers.getWebContentByFriendlyUrl(riplayWebContentPrefix + "disclaimer", themeDisplay);
...
//		renderRequest.setAttribute("riplayLink", riplayLink.getLinkHref());
		...
		// Link My Insurance
		MasterGlobalVariable myInsuranceLink = MasterGlobalVariableLocalServiceUtil
				.getActiveGlobalVariableByGroup("IT6").stream()
				.filter(v -> v.getName().contains("Link My Insurance") && v.getActive() == 1).findFirst().orElse(null);
...
//		renderRequest.setAttribute("pmcrPaymentUrl", pmcrPaymentUrl);
	...
	private String generateLowerPremiPrice() {
		...
		List<StiPremi> _premis = StiPremiLocalServiceUtil.findByCurrencyAndTravellerType("idr", "individual");
		List<Map<String, Double>> resultContainer = new ArrayList<>();

		if (Validator.isNull(smartTravelIntPlans) || smartTravelIntPlans.size() < 1 || Validator.isNull(_premis)
				|| _premis.size() < 1) {
			return result;
		}
		...
		for (SmartTravelIntPlan plan : smartTravelIntPlans) {
			...
			for (StiPremi premi : _premis) {
				StiBenefit currentBenefit = null;
				try {
					currentBenefit = StiBenefitLocalServiceUtil.getStiBenefit(premi.getBenefitId());
				} catch (Exception e) {
					System.out.println(e.getMessage());
					continue;
				}
