...
var exportMultiple = function () {
  ...
  var _allrowids = Liferay.Util.listCheckedExcept(
    form,
    "<portlet:namespace />allRowIds"
  );
  console.log(_allrowids);
  if (form) {
...
Liferay.componentReady("<%=product%>ManagementToolbar").then(function (
  managementToolbar
) {
...
if (exportImport !== null) {
  exportImport.on("click", function () {
    if (
      confirm('<liferay-ui:message key="Are you sure to export all data?" />')
    ) {
      var fmExport = document.getElementById("<portlet:namespace />fmExport");
      var form = document.getElementById("<portlet:namespace />fm");
      if (form) {
        var cmd = fmExport.querySelector(
          "#<portlet:namespace /><%= Constants.CMD %>"
        );
