...
public class ImageUploadFileEntryHandler implements UploadFileEntryHandler {
	...
	protected FileEntry uploadImage(UploadPortletRequest uploadPortletRequest) throws PortalException {
		PortalException portalException = null;
			...
			if(getFolder == null) {
				// create new folder
				dlFolder = DLFolderLocalServiceUtil.addFolder(themeDisplay.getUserId(), themeDisplay.getScopeGroupId(), repositoryId, false, 0, "MyPage", "", false, serviceContext);
			} else {
				// get folder
	...
	private static final String _FOLDER_NAME = "MyPage";
