...
@Component(
		...
		property = {
			...
			"javax.portlet.display-name=Agent Dashboard Portofolio Transaksi",
...
public class AgentDashboardPortofolioTransaksiPortlet extends MVCPortlet{
...
//	private static LogHelper<Object> logs = new LogHelper<>();
	private static Log _log = LogFactoryUtil.getLog(AgentDashboardPortofolioTransaksiPortlet.class);
		...
		try {
			...
			if (authStatus) {
				...
				PortletRequestDispatcher dispatcher = getPortletContext().getRequestDispatcher(
						"/META-INF/resources/dashboard/portofolio/dashboard_portofolio_transaksi_keseluruhan.jsp");
				dispatcher.include(renderRequest, renderResponse);
