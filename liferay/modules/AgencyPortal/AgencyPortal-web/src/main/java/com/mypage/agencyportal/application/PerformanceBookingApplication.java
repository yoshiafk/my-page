...
@Component(
	property = {
		JaxrsWhiteboardConstants.JAX_RS_APPLICATION_BASE + "=/scorecard",
		JaxrsWhiteboardConstants.JAX_RS_NAME + "=PerformanceBooking.Rest",
		"auth.verifier.guest.allowed=true",
		"liferay.access.control.disable=true"
	},
	service = Application.class
)
public class PerformanceBookingApplication extends Application {
...
//			e.printStackTrace();
	...
	@Path("/clear-session")
	@Produces("application/json")
	public String clearSessionAction(@QueryParam("a") String params) {
		JSONObject _response = JSONFactoryUtil.createJSONObject();
		try {
			@SuppressWarnings("static-access")
			String param = EncryptionHelper.decrypt(params);
			...
			String[] paramSplit = param.split("\\|");
			String deptCode = paramSplit[1];
			boolean deleteAgentToken = _agentTokenLocalService.deleteAgentTokenByDeptCode(deptCode);
			if(deleteAgentToken) {
				_response.put("status", true);
				_response.put("message", "Session was successfully deleted");
			} else {
				_response.put("status", false);
				_response.put("message", "Failed to delete session");
			}
