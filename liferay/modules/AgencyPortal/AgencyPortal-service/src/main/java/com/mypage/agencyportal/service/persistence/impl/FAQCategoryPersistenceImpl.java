...
import com.mypage.agencyportal.model.impl.FAQCategoryImpl;
import com.mypage.agencyportal.model.impl.FAQCategoryModelImpl;
import com.mypage.agencyportal.service.persistence.FAQCategoryPersistence;
import com.mypage.agencyportal.service.persistence.FAQCategoryUtil;
...
/**
 * The persistence implementation for the faq category service.
 *
 * <p>
 ...
 * @generated
 */
@Component(service = FAQCategoryPersistence.class)
public class FAQCategoryPersistenceImpl
	extends BasePersistenceImpl<FAQCategory> implements FAQCategoryPersistence {

	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use <code>FAQCategoryUtil</code> to access the faq category persistence. Modify <code>service.xml</code> and rerun ServiceBuilder to regenerate this class.
	 */
	public static final String FINDER_CLASS_NAME_ENTITY =
		FAQCategoryImpl.class.getName();
	...
	public FAQCategoryPersistenceImpl() {
		...
		setModelImplClass(FAQCategoryImpl.class);
	...
	public void cacheResult(List<FAQCategory> faqCategories) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (faqCategories.size() > _valueObjectFinderCacheListThreshold))) {
	...
	/**
	 * Clears the cache for all faq categories.
	 *
	 * <p>
	...
	/**
	 * Clears the cache for the faq category.
	 *
	 * <p>
	 * The <code>EntityCache</code> and <code>FinderCache</code> are both cleared by this method.
	 * </p>
	...
	public void clearCache(FAQCategory faqCategory) {
		entityCache.removeResult(FAQCategoryImpl.class, faqCategory);
	}
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			entityCache.removeResult(FAQCategoryImpl.class, primaryKey);
		}
	}
	...
	public FAQCategory create(long faqCategoryId) {
		...
		faqCategory.setPrimaryKey(faqCategoryId);

		faqCategory.setCompanyId(CompanyThreadLocal.getCompanyId());
	...
	public FAQCategory remove(long faqCategoryId)
		...
		return remove((Serializable)faqCategoryId);
	}

	/**
	 * Removes the faq category with the primary key from the database. Also notifies the appropriate model listeners.
	 *
	 * @param primaryKey the primary key of the faq category
	 * @return the faq category that was removed
	 * @throws NoSuchFAQCategoryException if a faq category with the primary key could not be found
	 */
	@Override
	public FAQCategory remove(Serializable primaryKey)
		...
		try {
			...
			FAQCategory faqCategory = (FAQCategory)session.get(
			...
			if (faqCategory == null) {
				...
				throw new NoSuchFAQCategoryException(
					_NO_SUCH_ENTITY_WITH_PRIMARY_KEY + primaryKey);
			}

			return remove(faqCategory);
		}
		catch (NoSuchFAQCategoryException noSuchEntityException) {
	...
	protected FAQCategory removeImpl(FAQCategory faqCategory) {
		...
		try {
			...
			if (!session.contains(faqCategory)) {
				faqCategory = (FAQCategory)session.get(
					FAQCategoryImpl.class, faqCategory.getPrimaryKeyObj());
			}
	...
	public FAQCategory updateImpl(FAQCategory faqCategory) {
		...
		if (!(faqCategory instanceof FAQCategoryModelImpl)) {
			InvocationHandler invocationHandler = null;

			if (ProxyUtil.isProxyClass(faqCategory.getClass())) {
				invocationHandler = ProxyUtil.getInvocationHandler(faqCategory);

				throw new IllegalArgumentException(
					"Implement ModelWrapper in faqCategory proxy " +
						invocationHandler.getClass());
			}

			throw new IllegalArgumentException(
				"Implement ModelWrapper in custom FAQCategory implementation " +
					faqCategory.getClass());
		}

		FAQCategoryModelImpl faqCategoryModelImpl =
			(FAQCategoryModelImpl)faqCategory;
		...
		if (isNew && (faqCategory.getCreateDate() == null)) {
			if (serviceContext == null) {
				faqCategory.setCreateDate(date);
			}
			else {
				faqCategory.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!faqCategoryModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				faqCategory.setModifiedDate(date);
			}
			else {
				faqCategory.setModifiedDate(
		...
		entityCache.putResult(FAQCategoryImpl.class, faqCategory, false, true);
	...
	/**
	 * Returns the faq category with the primary key or throws a <code>com.liferay.portal.kernel.exception.NoSuchModelException</code> if it could not be found.
	 *
	 * @param primaryKey the primary key of the faq category
	...
	public FAQCategory findByPrimaryKey(long faqCategoryId)
		...
		return findByPrimaryKey((Serializable)faqCategoryId);
	}
	...
	public FAQCategory fetchByPrimaryKey(long faqCategoryId) {
		return fetchByPrimaryKey((Serializable)faqCategoryId);
	}
	...
	public List<FAQCategory> findAll(
		...
		if (useFinderCache) {
			list = (List<FAQCategory>)finderCache.getResult(
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
				...
				sb.append(_SQL_SELECT_FAQCATEGORY);
			...
			try {
				...
				list = (List<FAQCategory>)QueryUtil.list(
	...
	public int countAll() {
		...
		if (count == null) {
			...
			try {
				...
				Query query = session.createQuery(_SQL_COUNT_FAQCATEGORY);
	...
	protected String getSelectSQL() {
		return _SQL_SELECT_FAQCATEGORY;
	}
	...
	/**
	 * Initializes the faq category persistence.
	 */
	@Activate
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			ArgumentsResolver.class, new FAQCategoryModelArgumentsResolver(),
			MapUtil.singletonDictionary(
				"model.class.name", FAQCategory.class.getName()));
		...
		_setFAQCategoryUtilPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		_setFAQCategoryUtilPersistence(null);
	...
	private void _setFAQCategoryUtilPersistence(
		FAQCategoryPersistence faqCategoryPersistence) {

		try {
			Field field = FAQCategoryUtil.class.getDeclaredField(
				"_persistence");

			field.setAccessible(true);

			field.set(null, faqCategoryPersistence);
		}
	...
	private static final String _SQL_SELECT_FAQCATEGORY =
		"SELECT faqCategory FROM FAQCategory faqCategory";

	private static final String _SQL_COUNT_FAQCATEGORY =
		"SELECT COUNT(faqCategory) FROM FAQCategory faqCategory";

	private static final String _ORDER_BY_ENTITY_ALIAS = "faqCategory.";

	private static final String _NO_SUCH_ENTITY_WITH_PRIMARY_KEY =
		"No FAQCategory exists with the primary key ";

	private static final Log _log = LogFactoryUtil.getLog(
		FAQCategoryPersistenceImpl.class);
	...
	private static class FAQCategoryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			FAQCategoryModelImpl faqCategoryModelImpl =
				(FAQCategoryModelImpl)baseModel;

			long columnBitmask = faqCategoryModelImpl.getColumnBitmask();

			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(faqCategoryModelImpl, columnNames, original);
			}
			...
			if (finderPathColumnBitmask == null) {
				...
				for (String columnName : columnNames) {
					finderPathColumnBitmask |=
						faqCategoryModelImpl.getColumnBitmask(columnName);
				}
		...
		private static Object[] _getValue(
			FAQCategoryModelImpl faqCategoryModelImpl, String[] columnNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = faqCategoryModelImpl.getColumnOriginalValue(
						columnName);
				}
				else {
					arguments[i] = faqCategoryModelImpl.getColumnValue(
						columnName);
				}
			}
