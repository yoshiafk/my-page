...
if(premiId > 0) {
	_premi = IntTravelStandardNewPremiLocalServiceUtil.getIntTravelStandardNewPremi(premiId);
}
%>
					...
					<div class="sheet-section">
						<aui:fieldset-group markupView="lexicon">
							<aui:fieldset cssClass="pt-4 pl-4">
								...
								<aui:row>			
									<aui:col width="50">
										<aui:select label="Travel Type" name="travel_type" required="true" showEmptyOption="true">
								            <aui:option value="single" selected="<%=_premi!=null && _premi.getTravelType().equals("single") %>" >Single</aui:option>
								            <aui:option value="annual" selected="<%=_premi!=null && _premi.getTravelType().equals("annual") %>" >Annual</aui:option>
								        </aui:select>
									</aui:col>										
								</aui:row>
								...
								<aui:row>
									<aui:col width="50">
										<aui:input label="Adult" name="spouse" type="number" value="<%= _premi!=null?_premi.getSpouse():"" %>" />
									</aui:col>					
									<aui:col width="50">
										<aui:input label="Children" name="child" type="number" value="<%= _premi!=null?_premi.getChild():"" %>" />
									</aui:col>
								</aui:row>
								...
								<aui:row>
									<aui:col width="50">
									<%
										String formattedAdditionalPrice = "";
									
										if(_premi!=null) {
											DecimalFormat decimalFormat = new DecimalFormat("#,##0.##");
											formattedAdditionalPrice = decimalFormat.format(_premi.getAdditionalPrice());				
										}
									%>
										<aui:input label="Additional Price" name="additional_price" type="text" value="<%= formattedAdditionalPrice %>" />
									</aui:col>
								</aui:row>
																														
							</aui:fieldset>
						</aui:fieldset-group>
						<aui:input name="premiId" type="hidden" value="<%= premiId > 0 && _premi != null ? _premi.getIntTravelStandardNewPremiId() : "" %>" />
						<aui:button-row>
